...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($name && $address && $email && $section && $contact && $password && $cpassword) {
        ...
        $check_email = mysqli_query($connection, "SELECT * FROM mytbl WHERE email = '$email_safe'");
        $check_email_row = mysqli_num_rows($check_email);

        if ($check_email_row > 0) {
            $emailErr = "Email is already registered!";
        } else {
            $query = mysqli_query($connection, "INSERT INTO mytbl (name, address, email, section, contact, password, account_type) VALUES ('$name', '$address', '$email', '$section', '$contact', '$cpassword', '2')");
